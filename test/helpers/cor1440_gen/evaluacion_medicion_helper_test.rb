...
module Cor1440Gen
  class EvaluacionMedicionHelperTest < ActionView::TestCase
    ...
    test "números e identificadores" do
      # números
      assert_in_delta(1.0, evalua_expresion_medicion(" 1 ", nil))
      assert_in_delta(1.0, evalua_expresion_medicion("\n1,0\t", nil))
      assert_in_delta(-2.0, evalua_expresion_medicion("-2", nil))
      assert_in_delta(-2.0, evalua_expresion_medicion("-2,0", nil))
      # identificadores
      assert_nil evalua_expresion_medicion("m", nil)
      assert_in_delta(1.0, evalua_expresion_medicion("m", { "m" => 1.0 }))
      assert_in_delta(-1.0, evalua_expresion_medicion("-m", { "m" => 1.0 }))
      # paréntesis
      assert_in_delta(1.0, evalua_expresion_medicion("(m)", { "m" => 1.0 }))
      assert_in_delta(-1.0, evalua_expresion_medicion("-(m)", { "m" => 1.0 }))
    ...
    test "aplicación funcional" do
      ...
      assert_equal 2, evalua_expresion_medicion(
        ...
        {
          "Actividades" => [
            { Asistentes: [1, 2], Organizaciones: [5] },
    ...
    test "binarias y paréntesis" do
      ...
      assert_in_delta(0.5, evalua_expresion_medicion("1/2", nil))
    ...
    test "trienal_cinep_2021" do
      ...
      assert_equal 50, evalua_expresion_medicion(
        'cuenta(
         ...
         )*100/cuenta(Escuelas_rurales_lineabase_2021)',
        con,
      )
