      ...
      module Persona
        ...
        included do
          ...
          def importa(datosent, datossal, menserror, opciones = {})
            ...
            importa_msip(ndatosent, datossal, menserror, opciones)
          end
          ...
          def complementa_importa(ulteditor_id, datossal, menserror, opciones)
            ...
            datossal[:respuestafor].each do |fid, r|
              ...
              if r.errors.messages && r.errors.messages.count > 0
                menserror << "  Error al salvar respuestafor de formulario #{fid}: #{r.errors.messages}."
              end
              ...
              if pf.count != 1
                ...
                b = Cor1440Gen::Beneficiariopf.where(proyectofinanciero_id:
                                                 pf.take.id)
                if b.count == 0
                  proyectofinanciero_ids << pf.take.id
                end
            ...
            datossal[:caracterizacion].each do |fid, c|
              if datossal[:respuestafor][fid].id
                ...
                if c.errors.messages && c.errors.messages.count > 0
                  menserror << "  Error al salvar caracterizacion de formulario #{fid}: #{c.errors.messages}."
                end
              else
              ...
              if datossal[:respuestafor][fid].id
                cam.each do |_cid, v|
                  ...
                  if v.errors.messages && v.errors.messages.count > 0
                    menserror << "  Error al salvar valor de campo #{v.campo_id} de formulario #{fid}: #{v.errors.messages}."
                  end
                end
              else
